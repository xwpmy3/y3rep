...
uninstall() {
    ...
    systemctl stop s-ui-new
    systemctl disable s-ui-new
    rm /etc/systemd/system/s-ui-new.service -f
    ...
    echo ""
    echo -e "Uninstalled Successfully, If you want to remove this script, then after exiting the script run ${green}rm /root/s-ui-new -f${plain} to delete it."
    echo ""
...
set_admin() {
    ...
    /root/s-ui-new/sui admin -username ${config_account} -password ${config_password}
    before_show_menu
}

view_admin() {
    /root/s-ui-new/sui admin -show
    before_show_menu
}
...
reset_setting() {
    ...
    if [[ $? == 0 ]]; then
        /root/s-ui-new/sui setting -reset
    fi
    before_show_menu
}
...
set_setting() {
    ...
    /root/s-ui-new/sui setting ${params}
    before_show_menu
}

view_setting() {
    /root/s-ui-new/sui setting -show
    view_uri
    before_show_menu
}

view_uri() {
    info=$(/root/s-ui-new/sui uri)
    if [[ $? != 0 ]]; then
...
show_usage() {
    ...
    echo -e "s-ui start        - Start s-ui-new"
    echo -e "s-ui stop         - Stop s-ui-new"
    echo -e "s-ui restart      - Restart s-ui-new"
    echo -e "s-ui status       - Current Status of s-ui-new"
    echo -e "s-ui enable       - Enable Autostart on OS Startup"
    echo -e "s-ui disable      - Disable Autostart on OS Startup"
    echo -e "s-ui log          - Check s-ui-new Logs"
...
————————————————————————————————
  ${green}11.${plain} s-ui-new Start
  ${green}12.${plain} s-ui-new Stop
  ${green}13.${plain} s-ui-new Restart
  ${green}14.${plain} s-ui-new Check State
  ${green}15.${plain} s-ui-new Check Logs
  ${green}16.${plain} s-ui-new Enable Autostart
  ${green}17.${plain} s-ui-new Disable Autostart
————————————————————————————————
...
————————————————————————————————
 "
    ...
    10)
        ...
        ;;
    11)
        check_install && start s-ui-new
        ;;
    12)
        check_install && stop s-ui-new
        ;;
    13)
        check_install && restart s-ui-new
        ;;
    14)
        check_install && status s-ui-new
        ;;
    15)
        check_install && show_log s-ui-new
        ;;
    16)
        check_install && enable s-ui-new
        ;;
    17)
        check_install && disable s-ui-new
        ;;
    18)
        bbr_menu
        ;;
...
if [[ $# > 0 ]]; then
    case $1 in
    "start")
        check_install 0 && start s-ui-new 0
        ;;
    "stop")
        check_install 0 && stop s-ui-new 0
        ;;
    "restart")
        check_install 0 && restart s-ui-new 0
        ;;
    "status")
        check_install 0 && status 0
        ;;
    "enable")
        check_install 0 && enable s-ui-new 0
        ;;
    "disable")
        check_install 0 && disable s-ui-new 0
        ;;
    "log")
        check_install 0 && show_log s-ui-new 0
        ;;
    "update")
