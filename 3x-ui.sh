...
uninstall() {
    ...
    rm /etc/systemd/system/3x-ui.service -f
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        /root/3x-ui/x-ui setting -username ${config_account} -password ${config_password} -resetTwoFactor false >/dev/null 2>&1
    else
        /root/3x-ui/x-ui setting -username ${config_account} -password ${config_password} -resetTwoFactor true >/dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi
    
    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the 3x-ui panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    /root/3x-ui/x-ui setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /root/3x-ui/x-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(/root/3x-ui/x-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(/root/3x-ui/x-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /root/3x-ui/x-ui setting -port ${port}
...
show_log() {
    ...
    0)
        ...
        ;;
    1)
        journalctl -u 3x-ui -e --no-pager -f -p debug
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        ;;
...
check_status() {
    ...
    temp=$(systemctl status 3x-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled 3x-ui)
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    /root/3x-ui/x-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
ssl_cert_issue() {
    local existing_webBasePath=$(/root/3x-ui/x-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/root/3x-ui/x-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    ...
    *)
    ...
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; 3x-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
        1)
            LOGI "Reloadcmd is: systemctl reload nginx ; 3x-ui restart"
            reloadCmd="systemctl reload nginx ; 3x-ui restart"
            ;;
        2)  
            LOGD "It's recommended to put 3x-ui restart at the end, so it won't raise an error if other services fails"
            read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; 3x-ui restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(/root/3x-ui/x-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/root/3x-ui/x-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(/root/3x-ui/x-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(/root/3x-ui/x-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(/root/3x-ui/x-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            /root/3x-ui/x-ui setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        /root/3x-ui/x-ui setting -listenIP 0.0.0.0 >/dev/null 2>&1
