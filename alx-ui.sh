...
uninstall() {
    ...
    systemctl stop alx-ui
    systemctl disable alx-ui
    rm /etc/systemd/system/alx-ui.service -f
...
reset_user() {
    ...
    /root/alx-ui/x-ui setting -username admin -password admin
...
reset_webbasepath() {
    ...
    /root/alx-ui/x-ui setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /root/alx-ui/x-ui setting -reset
...
check_config() {
    info=$(/root/alx-ui/x-ui setting -show true)
    if [[ $? != 0 ]]; then
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /root/alx-ui/x-ui setting -port ${port}
...
show_log() {
    ...
    0)
        ...
        ;;
    1)
        journalctl -u alx-ui -e --no-pager -f -p debug
        if [[ $# == 0 ]]; then
        before_show_menu
        fi
        ;;
...
check_status() {
    if [[ ! -f /etc/systemd/system/alx-ui.service ]]; then
        return 2
    fi
    temp=$(systemctl status alx-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled alx-ui)
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    /root/alx-ui/x-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
